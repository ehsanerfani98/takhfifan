...
<div class="w-full lg:w-1/3">
    <div class="bg-white rounded-2xl shadow-lg p-6 fade-in">
        ...
        <div class="mb-6">
            <div class="flex justify-between mb-2">
                <span class="text-xs font-medium text-blue-600">مرحله <span id="currentStepNumber">1</span>
                    از 6</span>
                <span class="text-xs font-medium text-gray-500">کارشناسی خودرو</span>
            </div>
        ...
        <!-- تیتر فرم -->
        <div class="mb-6">
            <h2 class="text-xl font-bold text-gray-800">ماشین خود را با اطمینان کارشناسی کنید.</h2>
        ...
        <div id="formSteps">
            ...
            <div class="step hidden" data-step="5">
                ...
                <!-- انتخاب تاریخ -->
                <div class="mb-6">
                    <h3 class="text-md font-semibold mb-3 text-gray-700">تاریخ مراجعه</h3>
                    <div id="dateList" class="grid grid-cols-4 gap-2 mb-4">
                        <!-- تاریخ‌ها به صورت دینامیک تولید می‌شوند -->
                    </div>
                </div>

                <!-- انتخاب زمان -->
                <div class="mb-6">
                    <h3 class="text-md font-semibold mb-3 text-gray-700">ساعت مراجعه</h3>
                    <div class="relative">
                        <select id="timeSelect"
                            class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-all appearance-none bg-white">
                            <option value="">انتخاب ساعت</option>
                            <!-- زمان‌ها به صورت دینامیک تولید می‌شوند -->
                        </select>
                        <div
                ...
                <div class="flex justify-between">
                    ...
                    <button
                        class="next flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-700 text-white py-3 px-6 rounded-xl shadow-md hover:shadow-lg transition-all">
                ...
                <div class="bg-gray-50 rounded-xl p-5 mb-6">
                    <ul id="preview" class="space-y-3 text-gray-700">
                        <li class="flex justify-between items-center pb-2 border-b border-gray-200">
                            <span class="font-medium">کارکرد (کیلومتر):</span>
                        ...
                        <li class="flex justify-between items-center pb-2 border-b border-gray-200">
                            <span class="font-medium">تاریخ مراجعه:</span>
                            <span id="previewDate" class="font-bold">-</span>
                        </li>
                        <li class="flex justify-between items-center">
                            <span class="font-medium">زمان مراجعه:</span>
                            <span id="previewTime" class="font-bold">-</span>
                        </li>
                    </ul>
                </div>
                <div class="flex justify-between">
                    ...
                    <button id="submitForm"
                        class="bg-gradient-to-r from-green-600 to-green-700 text-white py-3 px-6 rounded-xl shadow-md hover:shadow-lg transition-all flex items-center gap-2 btn-disabled">
    ...
    <script>
        ...
        function loadFormData() {
            ...
            if (urlParams.has('brand') && urlParams.has('brand_id') &&
                ...
                urlParams.has('year') && urlParams.has('type')) {
                ...
                if (urlParams.has('kilometer')) {
                    ...
                    if (kilometer) {
                        kilometer.value = data.kilometer;
                    }
                }

                // تاریخ و زمان را اگر وجود دارد ذخیره کن
                if (urlParams.has('visit_date')) {
                    data.selectedDate = urlParams.get('visit_date');
                    localStorage.setItem("selectedDate", data.selectedDate);

                    // ایجاد نمایش تاریخ شمسی
                    const visitDate = new Date(data.selectedDate);
                    ...
                    const persianMonths = ['فروردین', 'اردیبهشت', 'خرداد', 'تیر', 'مرداد', 'شهریور', 'مهر', 'آبان', 'آذر',
                        'دی', 'بهمن', 'اسفند'
                    ];

                    const dayName = persianDays[visitDate.getDay()];
                    const monthName = persianMonths[jalaaliDate.jm - 1];
                    data.selectedDateDisplay = `${dayName} ${jalaaliDate.jd} ${monthName}`;
                    localStorage.setItem("selectedDateDisplay", data.selectedDateDisplay);
                }

                if (urlParams.has('visit_time')) {
                    data.selectedTime = urlParams.get('visit_time');
                    localStorage.setItem("selectedTime", data.selectedTime);

                    // ایجاد نمایش زمان
                    const timeSlots = generateTimeSlots();
                    const selectedSlot = timeSlots.find(slot => slot.value === data.selectedTime);
                    if (selectedSlot) {
                        data.selectedTimeDisplay = selectedSlot.display;
                        localStorage.setItem("selectedTimeDisplay", data.selectedTimeDisplay);
                    }
                }
            ...
            if (savedData) {
                ...
                // مقداردهی فیلد کیلومتر از داده‌های ذخیره شده
                if (data.kilometer && kilometer) {
                    kilometer.value = data.kilometer;
                }

                // مقداردهی انتخاب‌های تاریخ و زمان از داده‌های ذخیره شده
                if (data.selectedDate && data.selectedTime) {
                    const timeSelect = document.getElementById('timeSelect');
                    if (timeSelect) {
                        // پیدا کردن گزینه مربوطه در select
                        const timeSlots = generateTimeSlots();
                        const selectedSlot = timeSlots.find(slot => slot.value === data.selectedTime);
                        if (selectedSlot) {
        ...
        function showStep(n) {
            ...
            // اگر مرحله تاریخ و زمان است، تاریخ‌ها و زمان‌ها را رندر کن
            if (n === 5) {
                renderJalaaliDates();
                renderTimeSlots();
            }

            // اگر مرحله پیش‌نمایش است، داده‌ها را به روز کن
            if (n === 6) {
                previewData();
                checkFormCompletion();
            }
        ...
        // تولید تاریخ‌های شمسی 7 روز آینده
        function generateJalaaliDates() {
            const dates = [];
            ...
            for (let i = 0; i < 7; i++) {
                ...
                const dayOfWeek = date.getDay(); // 0: یکشنبه, 1: دوشنبه, ...
                const dayName = persianDays[dayOfWeek];
                const monthName = persianMonths[jalaaliDate.jm - 1];

                // فرمت تاریخ: YYYY-MM-DD برای ذخیره سازی
                const dateString = date.toISOString().split('T')[0];

                dates.push({
                    display: `${dayName} ${jalaaliDate.jd} ${monthName}`,
                    value: dateString,
                    day: dayName,
                    dayNumber: jalaaliDate.jd,
                    month: monthName,
                    jalaali: jalaaliDate
                });
            }
        ...
        function generateTimeSlots() {
            ...
            for (let hour = 9; hour <= 18; hour++) {
                ...
                timeSlots.push({
                    display: `${startHour}:۰۰ تا ${endHour}:۰۰`,
        ...
        function renderJalaaliDates() {
            const dateList = document.getElementById("dateList");
            dateList.innerHTML = "";

            const dates = generateJalaaliDates();

            dates.forEach(date => {
                const button = document.createElement("button");
                button.type = "button";
                button.className =
                    "dateBtn border-2 border-gray-200 p-3 rounded-lg hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all duration-200 text-center text-sm";
                button.innerHTML = `
            <div class="font-medium">${date.day}</div>
            <div class="text-xs text-gray-600 mt-1">${date.dayNumber} ${date.month}</div>
        `;
                button.dataset.date = date.value;

                // اگر این تاریخ قبلاً انتخاب شده بود، آن را انتخاب شده نشان بده
                if (data.selectedDate === date.value) {
                    button.classList.add('date-selected', 'bg-blue-500', 'text-white', 'border-blue-500');
                    button.classList.remove('border-gray-200', 'text-gray-800');
                }

                button.onclick = function() {
                    // مورد اول: فقط می‌توان یک روز را انتخاب کرد
                    // حذف انتخاب قبلی از همه دکمه‌ها
                    document.querySelectorAll('.dateBtn').forEach(btn => {
                        btn.classList.remove('date-selected', 'bg-blue-500', 'text-white',
                            'border-blue-500');
                    ...
                    this.classList.add('date-selected', 'bg-blue-500', 'text-white', 'border-blue-500');

                    data.selectedDate = this.dataset.date;
                    data.selectedDateDisplay = `${date.day} ${date.dayNumber} ${date.month}`;
                    localStorage.setItem("selectedDate", data.selectedDate);
                    localStorage.setItem("selectedDateDisplay", data.selectedDateDisplay);
                    saveFormData();

                    // بررسی تکمیل بودن فرم
                    checkFormCompletion();
                };
        ...
        function renderTimeSlots() {
            ...
            timeSlots.forEach(slot => {
                ...
                // اگر این زمان قبلاً انتخاب شده بود، آن را انتخاب شده نشان بده
                if (data.selectedTime === slot.value) {
            ...
            // اضافه کردن event listener برای تغییر زمان
            timeSelect.addEventListener('change', function() {
                const selectedSlot = timeSlots.find(slot => slot.value === this.value);
                if (selectedSlot) {
                    data.selectedTime = this.value;
                    data.selectedTimeDisplay = selectedSlot.display;
                    localStorage.setItem("selectedTime", data.selectedTime);
                    localStorage.setItem("selectedTimeDisplay", data.selectedTimeDisplay);
                    saveFormData();
        ...
        // بررسی تکمیل بودن فرم برای فعال کردن دکمه ثبت
        function checkFormCompletion() {
            ...
            if (isFormComplete) {
                submitFormBtn.classList.remove('btn-disabled');
            } else {
                submitFormBtn.classList.add('btn-disabled');
            }
        ...
        function renderBrands(brands) {
            ...
            brands.forEach(brand => {
                const button = document.createElement("button");
                button.className =
                    "brandBtn border-2 border-gray-200 p-4 rounded-xl hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all duration-200 flex flex-col items-center justify-center gap-2";
                button.innerHTML = `
            <img class="h-10" src="${brand.icon}">
            <span class="font-medium">${brand.title}</span>
        `;
                button.onclick = () => {
                    // ذخیره کارکرد قبل از رفتن به مرحله بعد
                    data.kilometer = kilometer.value;
                    localStorage.setItem("kilometer", data.kilometer);
        ...
        function renderModels() {
            ...
            currentModels.forEach(model => {
                ...
                button.innerHTML = `
            <div class="border-2 border-gray-200 p-4 rounded-xl hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all duration-200 flex flex-col items-center justify-center gap-2">
        ...
        function renderYears(years) {
            ...
            years.forEach(year => {
                const button = document.createElement("button");
                button.className =
                    "yearBtn border-2 border-gray-200 p-3 rounded-lg hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all duration-200";
        ...
        function renderTypes(types) {
            ...
            types.forEach(type => {
                const button = document.createElement("button");
                button.className =
                    "typeBtn border-2 border-gray-200 p-4 rounded-xl hover:border-blue-500 hover:text-blue-500 hover:bg-blue-50 transition-all duration-200 flex flex-col items-center justify-center gap-2";
                button.innerHTML = `
        ...
        `;
                button.onclick = () => {
                    ...
                    saveFormData();
                    showStep(5); // رفتن به مرحله تاریخ و زمان
                };
        ...
        function previewData() {
            ...
            document.getElementById("previewDate").textContent = data.selectedDateDisplay || localStorage.getItem(
                'selectedDateDisplay') || '-';
            document.getElementById("previewTime").textContent = data.selectedTimeDisplay || localStorage.getItem(
                'selectedTimeDisplay') || '-';
        }

        // ارسال اطلاعات به سرور (آپدیت شده)
        async function submitFormData() {
            const submitBtn = document.getElementById("submitForm");

            // بررسی نهایی که همه فیلدها پر شده باشند
            ...
            try {
                // ارسال اطلاعات با تیپ و کارکرد و تاریخ و زمان
                $.ajax({
                    ...
                    data: {
                        ...
                        visit_time: localStorage.getItem('selectedTime') || '',
                        request_type: 'carinspection',
                        _token: CSRF_TOKEN
                    },
                    success: function(response) {
                        ...
                        localStorage.removeItem('selectedDateDisplay');
                        localStorage.removeItem('selectedTime');
                        localStorage.removeItem('selectedTimeDisplay');
        ...
        document.getElementById("submitForm").onclick = async () => {
            // بررسی نهایی که تاریخ و زمان انتخاب شده باشد
            if (!data.selectedDate || !data.selectedTime) {
                alert('لطفاً تاریخ و زمان مراجعه را انتخاب کنید.');
                return;
            }
        ...
        // مدیریت تغییرات فیلد کیلومتر
        if (kilometer) {
            kilometer.addEventListener('input', function() {
                data.kilometer = this.value;
                localStorage.setItem("kilometer", this.value);
                saveFormData();
                checkFormCompletion();
            });
        }
