...
<head>
    ...
    <style>
        ...
        * {
        ...
        .banner-section {
            flex: 1;
            background: linear-gradient(135deg, #4b87ff 0%, #664bff 100%);
            padding: 40px 20px;
            display: flex;
        ...
        .input-container input.is-invalid~.invalid-feedback {
            display: block;
        }
...
<body>
        ...
        <div class="form-section">
            ...
            <div id="login-step-1" class="login-step">
                <form id="send-otp-form">
                    @if (get_setting('login_type') == 'mobile')
                    ...
                    <div class="form-group">
                        @if (boolval(get_setting('sms_status')))
                            <button type="submit" id="send-otp-btn" class="btn btn-primary">
                                <span class="spinner-border d-none" id="send-spinner"></span>
                                دریافت کد تایید از طریق نوتیفیکیشن</button>
                        @else
            ...
            <div id="login-step-2" class="login-step d-none">
                <div class="alert alert-info">
                    ...
                    @else
                        <i class="fas fa-info-circle"></i> کد تایید به ایمیل <span id="email-display"></span> ارسال شد.
                    @endif
                </div>
                ...
                <form id="verify-otp-form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-container">
                            <input type="text" id="code" name="code" placeholder="کد 6 رقمی" required
                                maxlength="6">
            ...
            <div id="login-step-3" class="login-step d-none">
                <form id="password-otp-form">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        <button type="submit" class="btn btn-primary" id="send-password">
                            <span class="spinner-border d-none" id="send-spinner-pass"></span>
                            ورود
                        </button>
                    </div>
                </form>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#send-otp-form').on('submit', function(e) {
                ...
                @else
                    ...
                    if (!validateEmail(email)) {
                        showError('email-error', 'لطفا اکانت جیمیل معتبر وارد کنید');
                        return;
                    }
                @endif
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            if (response.status == 'exist_email') {
            ...
            $('#password-otp-form').on('submit', function(e) {
                e.preventDefault();

                let email = $('#verify-email').val().trim();
                let password = $('#password').val();

                $('#send-spinner-pass').removeClass('d-none');
                $('#send-password').prop('disabled', true);

                $.ajax({
                    url: '/otp/send-password',
                    type: 'POST',
                    data: {
                        email: email,
                        password: password,
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            showError('code-error', response.message || 'رمز عبور صحیح نیست');
                        }
                    },
                    error: function(xhr) {
                        let errorMsg = 'خطا در ارسال رمز عبور';
                        if (xhr.responseJSON && xhr.responseJSON.message) {
                            errorMsg = xhr.responseJSON.message;
                        }
                    ...
                    complete: function() {
                        $('#send-spinner-pass').addClass('d-none');
            ...
            $('#verify-otp-form').on('submit', function(e) {
                ...
                @else
                    email = $('#verify-email').val().trim();
                    if (!validateEmail(email)) {
                        showError('code-error', 'اکانت جیمیل نامعتبر است');
                        return;
                    }
                @endif
