...
@section('actions')
    <a href="{{ route('services.index') }}" class="btn btn-primary btn-sm btn-icon-split">
        ...
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="icon" name="icon" rows="3">{{ old('icon', $service->icon) }}</textarea>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                    ...
                    {{-- <textarea class="form-control" id="description" name="description" rows="16">{{ old('description', $service->description) }}</textarea> --}}
                    <x-text-editor name="description" id="description" :value="old('description', $service->description)" />
                </div>

                <div class="mb-3">
                    <label for="rules" class="form-label">قوانین</label>
                    <x-text-editor name="rules" id="rules" :value="old('rules', $service->rules)" />
                </div>

                <div class="mb-3">
                    <label for="parent_id" class="form-label">سرویس مادر (اختیاری)</label>
                    <select class="form-control" id="parent_id" name="parent_id">
                        <option value="">بدون والد (سرویس اصلی)</option>
                        @foreach ($services as $srv)
                            @continue($srv->id === $service->id)
                            <option value="{{ $srv->id }}"
                                {{ old('parent_id', $service->parent_id) == $srv->id ? 'selected' : '' }}>
                                {{ $srv->name }}
                            </option>
                        @endforeach
                    </select>
